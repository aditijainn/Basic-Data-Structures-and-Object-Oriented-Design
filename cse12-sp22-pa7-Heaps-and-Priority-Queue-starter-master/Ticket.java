 ...
 * Sources used: None
 * 
 * Ticket file that simulates priority 
 * queue for autograder ticket system
 */
...
public class Ticket implements Comparable<Ticket>{
	...
	/**
	 * CompareTo method replicated for The order of precedence 
	 * is the order value of ticketType and then createdAt
	 * @param other - other ticket to compare to
	 * @return - value from comparing
	 */
	@Override
	public int compareTo(Ticket other){
		// returns 0 if equal order values
		if(getCreatedAt().equals(other.getCreatedAt()) 
		&& getTicketType().equals(other.getTicketType())) {
			return 0;
		}
		int time = this.getCreatedAt().compareTo
		(other.getCreatedAt());
		int type = orderMap.get(getTicketType()).compareTo
		(orderMap.get(other.getTicketType()));
		// returns type order value before time if not equal
		if(type != 0) {
			return type;
		}
		else { 
